  ...
  module Mocks
    describe "failing MockArgumentMatchers" do
      ...
      it "rejects non numeric" do
        expect(@double).to receive(:random_call).with(an_instance_of(Numeric))
        expect do
      ...
      it "rejects non string" do
        expect(@double).to receive(:random_call).with(an_instance_of(String))
        expect do
      ...
      it "matches against a Matcher" do
        ...
        expect do
          expect(@double).to receive(:msg).with(equal(3))
      ...
      it "fails hash_including with missing key" do
         expect do
           expect(@double).to receive(:msg).with(hash_including(:a => 1))
           @double.msg({})
      ...
      it "fails array_including when args aren't array" do
         expect do
           expect(@double).to receive(:msg).with(array_including(1,2,3))
           @double.msg(1,2,3)
      ...
      it "fails with zero arguments" do
        expect do
          expect(@double).to receive(:msg).with {|arg| expect(arg).to eq :received }
      ...
      it "fails with sensible message when args respond to #description" do
        ...
        expect do
          expect(@double).to receive(:msg).with(3)
          @double.msg arg
        end.to raise_error(RSpec::Mocks::MockExpectationError, "Double \"double\" received :msg with unexpected arguments\n  expected: (3)\n       got: (my_thing)")
      end
