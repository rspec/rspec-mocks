      ...
      shared_examples_for "resets partial mocks cleanly" do
        ...
        context "on a frozen object" do
          it "warns about being unable to remove the method double" do
            #One for this test, one for RSpec's own lifecycle
            expect_warning_without_call_site(/rspec-mocks was unable to restore the original `foo` method on #{object.inspect}/).twice
          ...
          it "includes the spec location in the warning" do
            ...
            #One for this test, one for RSpec's own lifecycle
            expect_warning_without_call_site(/#{RSpec::Core::Metadata.relative_path(__FILE__)}:#{line}/).twice
            RSpec::Mocks.with_temporary_scope do
              target.to receive(:foo).and_return(:baz)
              object.freeze
              object.foo
